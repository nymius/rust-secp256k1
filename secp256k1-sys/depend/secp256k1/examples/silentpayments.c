 ...
 *
 *      rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs(..., NULL, NULL);
 */
...
int main(void) {
    ...
    rustsecp256k1_v0_12_xonly_pubkey *tx_output_ptrs[N_OUTPUTS];
    ...
    rustsecp256k1_v0_12_pubkey unlabeled_spend_pubkey;
    ...
    ret = rustsecp256k1_v0_12_context_randomize(ctx, randomize);
    assert(ret);
    ...
    {
        size_t len = 33;
        rustsecp256k1_v0_12_pubkey label, labeled_spend_pubkey;
        unsigned int m = 1;

        /* Load Bob's spend public key */
        ret = rustsecp256k1_v0_12_ec_pubkey_parse(ctx,
         ...
         */
        ret = rustsecp256k1_v0_12_silentpayments_recipient_create_labeled_spend_pubkey(ctx, &labeled_spend_pubkey, &unlabeled_spend_pubkey, &label);
        assert(ret);
    ...
    /*** Sending (Alice) ***/
    {
        ...
        for (i = 0; i < N_OUTPUTS; i++) {
            printf("    ");
            ret = rustsecp256k1_v0_12_xonly_pubkey_serialize(ctx,
                serialized_xonly,
                &tx_outputs[i]
            );
            assert(ret);
    ...
    /*** Receiving ***/
    {
        unsigned char light_client_data33[33];
        {
             ...
             *        `rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_create`
             *     2. Call `rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs`
             *
             * Bob collects the prevouts data from the transaction inputs and
             * creates a `rustsecp256k1_v0_12_silentpayments_prevouts_summary` object. He uses
             ...
             * with light clients.
             */
            ret = rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_create(ctx,
                &prevouts_summary,
                smallest_outpoint,
            ...
            ret = rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_serialize(ctx,
                light_client_data33,
                33,
                &prevouts_summary,
                SECP256K1_EC_COMPRESSED
            ...
            ret = rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs(ctx,
                ...
                (const rustsecp256k1_v0_12_xonly_pubkey * const *)tx_output_ptrs, N_OUTPUTS,
                bob_scan_key,
                &prevouts_summary,
                &unlabeled_spend_pubkey,
            ...
            if (n_found_outputs > 0) {
                rustsecp256k1_v0_12_keypair kp;
                rustsecp256k1_v0_12_xonly_pubkey xonly_output;
                ...
                for (i = 0; i < n_found_outputs; i++) {
                     ...
                     * been added to the tweak in `rustsecp256k1_v0_12_silentpayments_found_output`.
                     *
                    ...
                    ret = rustsecp256k1_v0_12_ec_seckey_tweak_add(ctx, full_seckey, found_outputs[i].tweak);
                    ret &= rustsecp256k1_v0_12_keypair_create(ctx, &kp, full_seckey);
                    ret &= rustsecp256k1_v0_12_keypair_xonly_pub(
                        ctx,
                        &xonly_output,
                        NULL,
                     ...
                     */
                    assert(ret);
                    assert(rustsecp256k1_v0_12_xonly_pubkey_cmp(ctx, &xonly_output, &found_outputs[i].output) == 0);
        ...
        }
        {
             ...
             * `rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs` to find all of
            ...
            ret = rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_parse(ctx,
                &prevouts_summary,
                light_client_data33,
                33
            );
            ...
            n_found_outputs = 0;
            {
                rustsecp256k1_v0_12_xonly_pubkey *potential_output_ptrs[1];
                rustsecp256k1_v0_12_xonly_pubkey potential_outputs[1];
                ...
                ret = rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkeys(ctx,
                ...
                found = 0;
                for (i = 0; i < N_OUTPUTS; i++) {
                    if (rustsecp256k1_v0_12_xonly_pubkey_cmp(ctx, &potential_outputs[0], &tx_outputs[i]) == 0) {
                        found = 1;
                        break;
                    }
                }
