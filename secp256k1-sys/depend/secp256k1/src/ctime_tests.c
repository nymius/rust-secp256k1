...
static void run_tests(rustsecp256k1_v0_12_context *ctx, unsigned char *key) {
...
#endif
#ifdef ENABLE_MODULE_SILENTPAYMENTS
    rustsecp256k1_v0_12_xonly_pubkey generated_output;
    rustsecp256k1_v0_12_xonly_pubkey *generated_outputs[1];
    rustsecp256k1_v0_12_silentpayments_recipient recipient;
    const rustsecp256k1_v0_12_silentpayments_recipient *recipients[1];
    unsigned char outpoint_smallest[36] = { 0 };
    rustsecp256k1_v0_12_keypair taproot_seckey;
    const rustsecp256k1_v0_12_keypair *taproot_seckeys[1];
    const unsigned char *plain_seckeys[1];
    rustsecp256k1_v0_12_silentpayments_found_output *found_outputs[1];
    size_t n_found_outputs;
    const rustsecp256k1_v0_12_xonly_pubkey *tx_outputs[1];
    ...
    const rustsecp256k1_v0_12_xonly_pubkey *xonly_pubkeys[1];
    rustsecp256k1_v0_12_pubkey plain_pubkey;
    const rustsecp256k1_v0_12_pubkey *plain_pubkeys[1];
#endif
    ...
    CHECK(rustsecp256k1_v0_12_ec_pubkey_create(ctx, &recipient.scan_pubkey, key));
    key[31] ^= 1;
    CHECK(rustsecp256k1_v0_12_ec_pubkey_create(ctx, &recipient.spend_pubkey, key));
    key[31] ^= (1 << 1);
    recipient.index = 0;
    ...
    /* Set up secret keys */
    SECP256K1_CHECKMEM_UNDEFINE(key, 32);
    ret = rustsecp256k1_v0_12_keypair_create(ctx, &taproot_seckey, key);
    SECP256K1_CHECKMEM_DEFINE(&ret, sizeof(ret));
    CHECK(ret);
    key[31] ^= (1 << 2);
    ...
    ret = rustsecp256k1_v0_12_silentpayments_sender_create_outputs(ctx, generated_outputs, recipients, 1, outpoint_smallest, taproot_seckeys, 1, plain_seckeys, 1);
    CHECK(ret == 1);

    /* TODO: use non-confusing public key */
    ret = rustsecp256k1_v0_12_silentpayments_recipient_create_label(ctx, &recipient.spend_pubkey, label_tweak, key, 0);
    key[31] ^= (1 << 3);
    SECP256K1_CHECKMEM_DEFINE(&ret, sizeof(ret));
    CHECK(ret == 1);

    CHECK(rustsecp256k1_v0_12_keypair_xonly_pub(ctx, &xonly_pubkey, NULL, &taproot_seckey));
    SECP256K1_CHECKMEM_DEFINE(&xonly_pubkey, sizeof(xonly_pubkey));
    xonly_pubkeys[0] = &xonly_pubkey;
    ret = rustsecp256k1_v0_12_ec_pubkey_create(ctx, &plain_pubkey, plain_seckeys[0]);
    SECP256K1_CHECKMEM_DEFINE(&ret, sizeof(ret));
    CHECK(ret == 1);
    SECP256K1_CHECKMEM_DEFINE(&plain_pubkey, sizeof(plain_pubkey));
    plain_pubkeys[0] = &plain_pubkey;

    ret = rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(ctx, &public_data, outpoint_smallest, xonly_pubkeys, 1, plain_pubkeys, 1);
    CHECK(ret == 1);
    ...
    ret = rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs(ctx, found_outputs, &n_found_outputs, tx_outputs, 1, key, &public_data, &recipient.spend_pubkey, NULL, NULL);
    CHECK(ret == 1);

    /* TODO: this fails */
    /* CHECK(rustsecp256k1_v0_12_silentpayments_recipient_create_shared_secret(ctx, shared_secret, key, &public_data)); */
    /* TODO: test rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkey */
