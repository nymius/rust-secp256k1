...
typedef struct {
    ...
    rustsecp256k1_v0_12_fe* output_xonly;
} bench_data;
...
static void bench_ecmult_const_xonly(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        const rustsecp256k1_v0_12_ge* pubkey = &data->pubkeys[(data->offset1+i) % POINTS];
        const rustsecp256k1_v0_12_scalar* scalar = &data->scalars[(data->offset2+i) % POINTS];
        int known_on_curve = 1;
        rustsecp256k1_v0_12_ecmult_const_xonly(&data->output_xonly[i], &pubkey->x, NULL, scalar, known_on_curve);
    }
}
...
static void bench_ecmult_const_xonly_teardown(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        const rustsecp256k1_v0_12_gej* pubkey_gej = &data->pubkeys_gej[(data->offset1+i) % POINTS];
        const rustsecp256k1_v0_12_scalar* scalar = &data->scalars[(data->offset2+i) % POINTS];
        rustsecp256k1_v0_12_gej expected_gej;
        rustsecp256k1_v0_12_ecmult(&expected_gej, pubkey_gej, scalar, NULL);
        CHECK(rustsecp256k1_v0_12_gej_eq_x_var(&data->output_xonly[i], &expected_gej));
    }
}
...
int main(int argc, char **argv) {
    ...
    scratch_size = rustsecp256k1_v0_12_strauss_scratch_size(POINTS) + STRAUSS_SCRATCH_OBJECTS*ALIGNMENT;
    ...
    data.output_xonly = malloc(sizeof(rustsecp256k1_v0_12_fe) * (iters + 1));
