...
static void test_bad_scalar(void) {
    ...
    memcpy(s_overflow, rustsecp256k1_v0_12_group_order_bytes, 32);
...
#include "../../wycheproof/ecdh_secp256k1_test.h"
    ...
    for (t = 0; t < SECP256K1_ECDH_WYCHEPROOF_NUMBER_TESTVECTORS; t++) {
        ...
        parsed_ok = rustsecp256k1_v0_12_ec_pubkey_parse(CTX, &point, pk, testvectors[t].pk_len);
        ...
        CHECK(rustsecp256k1_v0_12_ecdh(CTX, output_ecdh, &point, sk, ecdh_hash_function_test_xpassthru, NULL) == 1);
        expected_shared_secret = &wycheproof_ecdh_shared_secrets[testvectors[t].shared_offset];

        CHECK(rustsecp256k1_v0_12_memcmp_var(output_ecdh, expected_shared_secret, testvectors[t].shared_len) == 0);
    }
}
