...
static void test_label_api(void) {
    rustsecp256k1_v0_12_pubkey l, s, ls, e; /* label pk, spend pk, labelled spend pk, expected labelled spend pk */
...
static void test_recipient_api(void) {
    ...
    pp[0] = &p;
    fp[0] = &f;
    CHECK(rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, pp, 1));

    /* Check null values are handled */
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, NULL, SMALLEST_OUTPOINT, tp, 1, pp, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, NULL, tp, 1, pp, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, NULL, 1, pp, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, NULL, 1));

    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_serialize(CTX, NULL, &pd));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_serialize(CTX, o, NULL));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_parse(CTX, NULL, o));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_parse(CTX, &pd, NULL));

    /* Check that malformed serializations are rejected */
    CHECK(rustsecp256k1_v0_12_silentpayments_recipient_public_data_parse(CTX, &pd, malformed) == 0);
    ...
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_serialize(CTX, o, &pd));
    ...
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 0, pp, 1));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, pp, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, NULL, 0, pp, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &pd, SMALLEST_OUTPOINT, NULL, 0, NULL, 0));

    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_create_shared_secret(CTX, NULL, ALICE_SECKEY, &pd));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_create_shared_secret(CTX, o, NULL, &pd));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_create_shared_secret(CTX, o, ALICE_SECKEY, NULL));

    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkey(CTX, NULL, o, &p, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkey(CTX, &t, NULL, &p, 0));
    CHECK_ILLEGAL(CTX, rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkey(CTX, &t, o, NULL, 0));
...
void run_silentpayments_test_vector_receive(const struct bip352_test_vector *test) {
    ...
    /* prepare the inputs */
    {
        ...
        ret = rustsecp256k1_v0_12_silentpayments_recipient_public_data_create(CTX, &public_data,
            test->outpoint_smallest,
    ...
    if (test->num_labels == 0) {
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_public_data_serialize(CTX, light_client_data, &public_data));
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_public_data_parse(CTX, &public_data_index, light_client_data));
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_create_shared_secret(CTX, shared_secret_lightclient, test->scan_seckey, &public_data_index));
        n_found = 0;
        {
            int found = 0;
            size_t k = 0;
            rustsecp256k1_v0_12_xonly_pubkey potential_output;

            while(1) {

                CHECK(rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkey(CTX,
                    &potential_output,
                ...
                found = 0;
                for (i = 0; i < test->num_to_scan_outputs; i++) {
                    if (rustsecp256k1_v0_12_xonly_pubkey_cmp(CTX, &potential_output, tx_outputs[i]) == 0) {
                        rustsecp256k1_v0_12_xonly_pubkey_serialize(CTX, found_outputs_light_client[n_found], &potential_output);
                        found = 1;
                        n_found++;
                        k++;
                        break;
        ...
        for (i = 0; i < n_found; i++) {
            match = 0;
            for (j = 0; j < test->num_found_output_pubkeys; j++) {
                if (rustsecp256k1_v0_12_memcmp_var(&found_outputs_light_client[i], test->found_output_pubkeys[j], 32) == 0) {
                    match = 1;
                    break;
                }
            }
