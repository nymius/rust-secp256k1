...
static void bench_silentpayments_scan_setup(void* arg) {
    ...
    CHECK(rustsecp256k1_v0_12_ec_pubkey_serialize(data->ctx, input_pubkey33, &pubkeylen, &input_pubkey, SECP256K1_EC_COMPRESSED));
    CHECK(rustsecp256k1_v0_12_silentpayments_recipient_public_data_parse(data->ctx, &data->public_data, input_pubkey33));
    CHECK(rustsecp256k1_v0_12_ec_pubkey_parse(data->ctx, &data->spend_pubkey, spend_pubkey, pubkeylen));
...
static void bench_silentpayments_output_scan(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_create_shared_secret(data->ctx,
            shared_secret,
            data->scan_key,
            &data->public_data
        ));
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkey(data->ctx,
            &xonly_output,
            shared_secret,
            &data->spend_pubkey,
            k
...
static void bench_silentpayments_full_tx_scan(void* arg, int iters) {
    int i;
    size_t n_found = 0;
    rustsecp256k1_v0_12_silentpayments_found_output *found_output_ptrs[4];
    const rustsecp256k1_v0_12_xonly_pubkey *tx_output_ptrs[4];
    bench_silentpayments_data *data = (bench_silentpayments_data*)arg;
