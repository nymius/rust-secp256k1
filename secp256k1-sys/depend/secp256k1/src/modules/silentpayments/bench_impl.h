...
typedef struct {
    ...
    rustsecp256k1_v0_12_pubkey spend_pubkeys[1];
    unsigned char scan_key[32];
    unsigned char input_pubkey33[33];
    rustsecp256k1_v0_12_xonly_pubkey tx_outputs[2];
    rustsecp256k1_v0_12_xonly_pubkey tx_inputs[2];
    rustsecp256k1_v0_12_silentpayments_found_output found_outputs[2];
...
static void bench_silentpayments_scan_setup(void* arg) {
    ...
    rustsecp256k1_v0_12_keypair input_keypair;
    rustsecp256k1_v0_12_pubkey input_pubkey;
    size_t pubkeylen = 33;
    ...
    for (i = 0; i < 2; i++) {
        CHECK(rustsecp256k1_v0_12_xonly_pubkey_parse(data->ctx, &data->tx_outputs[i], tx_outputs[i]));
    }
     ...
     */
    CHECK(rustsecp256k1_v0_12_keypair_create(data->ctx, &input_keypair, data->scalar));
    CHECK(rustsecp256k1_v0_12_keypair_pub(data->ctx, &input_pubkey, &input_keypair));
    CHECK(rustsecp256k1_v0_12_ec_pubkey_serialize(data->ctx, data->input_pubkey33, &pubkeylen, &input_pubkey, SECP256K1_EC_COMPRESSED));
    /* Create the input public keys for the full scan */
    CHECK(rustsecp256k1_v0_12_keypair_xonly_pub(data->ctx, &data->tx_inputs[0], NULL, &input_keypair));
    CHECK(rustsecp256k1_v0_12_xonly_pubkey_parse(data->ctx, &data->tx_inputs[1], static_tx_input));
    CHECK(rustsecp256k1_v0_12_ec_pubkey_parse(data->ctx, &data->spend_pubkeys[0], spend_pubkey, pubkeylen));
...
static void bench_silentpayments_output_scan(void* arg, int iters) {
    ...
    const rustsecp256k1_v0_12_pubkey *spend_ptrs[1];
    spend_ptrs[0] = &data->spend_pubkeys[0];

    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_12_xonly_pubkey *xonly_output_ptrs[1];
        rustsecp256k1_v0_12_xonly_pubkey xonly_outputs[1];
        xonly_output_ptrs[0] = &xonly_outputs[0];
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_parse(data->ctx, &prevouts_summary, data->input_pubkey33, 33));
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkeys(data->ctx,
            xonly_output_ptrs,
            data->scan_key,
            &prevouts_summary,
            spend_ptrs,
...
static void bench_silentpayments_full_tx_scan(void* arg, int iters, int use_labels) {
    int i;
    size_t n_found = 0;
    rustsecp256k1_v0_12_silentpayments_found_output *found_output_ptrs[2];
    const rustsecp256k1_v0_12_xonly_pubkey *tx_output_ptrs[2];
    const rustsecp256k1_v0_12_xonly_pubkey *tx_input_ptrs[2];
    bench_silentpayments_data *data = (bench_silentpayments_data*)arg;
    rustsecp256k1_v0_12_silentpayments_prevouts_summary prevouts_summary;
    const rustsecp256k1_v0_12_silentpayments_label_lookup label_lookup_fn = use_labels ? label_lookup : NULL;
    ...
    for (i = 0; i < iters; i++) {
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_create(data->ctx,
            &prevouts_summary,
            data->smallest_outpoint,
            tx_input_ptrs, 2,
            NULL, 0
        ));
        CHECK(rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs(data->ctx,
