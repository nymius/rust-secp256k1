 ...
 *
 *  Note: rustsecp256k1_v0_12_silentpayments_recipient_sort uses heap sort, which is unstable.
...
static int rustsecp256k1_v0_12_silentpayments_create_output_pubkey(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_xonly_pubkey *output_xonly, const unsigned char *shared_secret33, const rustsecp256k1_v0_12_pubkey *spend_pubkey, uint32_t k) {
    ...
    if (!rustsecp256k1_v0_12_pubkey_load(ctx, &output_ge, spend_pubkey)) {
...
int rustsecp256k1_v0_12_silentpayments_sender_create_outputs(
    ...
    for (i = 0; i < n_recipients; i++) {
        ...
        if (!rustsecp256k1_v0_12_silentpayments_create_output_pubkey(ctx, generated_outputs[recipients[i]->index], shared_secret, &recipients[i]->spend_pubkey, k)) {
            rustsecp256k1_v0_12_scalar_clear(&seckey_sum_scalar);
            rustsecp256k1_v0_12_memclear_explicit(&shared_secret, sizeof(shared_secret));
            return 0;
        }
...
int rustsecp256k1_v0_12_silentpayments_recipient_create_label(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_pubkey *label, unsigned char *label_tweak32, const unsigned char *scan_key32, uint32_t m) {
    rustsecp256k1_v0_12_sha256 hash;
...
int rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs(
    ...
    for (k = 0; k < n_tx_outputs; k++) {
        ...
        if (found) {
            ...
            rustsecp256k1_v0_12_scalar_get_b32(found_outputs[k]->tweak, &output_tweak_scalar);
            ...
            if (label_tweak != NULL) {
                 ...
                 */
                if (!rustsecp256k1_v0_12_ec_seckey_tweak_add(ctx, found_outputs[k]->tweak, label_tweak)) {
                    memset(found_outputs[k]->tweak, 0, 32);
                }
                rustsecp256k1_v0_12_pubkey_save(&found_outputs[k]->label, &label_ge);
            } else {
                found_outputs[k]->found_with_label = 0;
                /* Set the label public key with an invalid public key value. */
                memset(&found_outputs[k]->label, 0, sizeof(rustsecp256k1_v0_12_pubkey));
            }
