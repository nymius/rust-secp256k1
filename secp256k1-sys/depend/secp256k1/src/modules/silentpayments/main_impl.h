...
static const unsigned char rustsecp256k1_v0_12_silentpayments_prevouts_summary_magic[4] = { 0xa7, 0x1c, 0xd3, 0x5e };

/** Sort an array of silent payment recipients. This is used to group recipients by scan pubkey to
 *  ensure the correct values of k are used when creating multiple outputs for a recipient.
 *
 *  Note: rustsecp256k1_v0_12_ec_pubkey_cmp uses heap sort, which is unstable. Developers cannot and should not
...
static int rustsecp256k1_v0_12_silentpayments_calculate_input_hash_scalar(rustsecp256k1_v0_12_scalar *input_hash_scalar, const unsigned char *outpoint_smallest36, rustsecp256k1_v0_12_ge *pubkey_sum) {
    ...
    ret &= !rustsecp256k1_v0_12_scalar_is_zero(input_hash_scalar);
    return ret & !overflow;
}

static void rustsecp256k1_v0_12_silentpayments_create_shared_secret(const rustsecp256k1_v0_12_context *ctx, unsigned char *shared_secret33, const rustsecp256k1_v0_12_ge *public_component, const rustsecp256k1_v0_12_scalar *secret_component) {
    ...
    rustsecp256k1_v0_12_declassify(ctx, &ss, sizeof(ss));
...
static int rustsecp256k1_v0_12_silentpayments_create_output_tweak(rustsecp256k1_v0_12_scalar *output_tweak_scalar, const unsigned char *shared_secret33, uint32_t k) {
    rustsecp256k1_v0_12_sha256 hash;
     ...
     * an error to ensure strict compliance with BIP0352.
     */
    rustsecp256k1_v0_12_scalar_set_b32(output_tweak_scalar, hash_ser, &overflow);
    ret = !rustsecp256k1_v0_12_scalar_is_zero(output_tweak_scalar);
    /* Leaking this value would break indistinguishability of the transaction, so clear it. */
    rustsecp256k1_v0_12_memclear_explicit(hash_ser, sizeof(hash_ser));
    rustsecp256k1_v0_12_sha256_clear(&hash);
    return ret & !overflow;
}

static int rustsecp256k1_v0_12_silentpayments_create_output_pubkeys(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_xonly_pubkey **outputs_xonly, const unsigned char *shared_secret33, const rustsecp256k1_v0_12_pubkey **spend_pubkeys, size_t n_spend_pubkeys, uint32_t k) {
    rustsecp256k1_v0_12_ge output_ge;
    rustsecp256k1_v0_12_scalar output_tweak_scalar;
    size_t i;
     ...
     */
    if (!rustsecp256k1_v0_12_silentpayments_create_output_tweak(&output_tweak_scalar, shared_secret33, k)) {
        return 0;
    }
    for (i = 0; i < n_spend_pubkeys; i++) {
       if (!rustsecp256k1_v0_12_pubkey_load(ctx, &output_ge, spend_pubkeys[i])) {
           rustsecp256k1_v0_12_scalar_clear(&output_tweak_scalar);
           return 0;
       }
        ...
        */
       if (!rustsecp256k1_v0_12_eckey_pubkey_tweak_add(&output_ge, &output_tweak_scalar)) {
           rustsecp256k1_v0_12_scalar_clear(&output_tweak_scalar);
           return 0;
       };
       rustsecp256k1_v0_12_xonly_pubkey_save(outputs_xonly[i], &output_ge);
    }
...
int rustsecp256k1_v0_12_silentpayments_sender_create_outputs(
    ...
    size_t i, k;
    rustsecp256k1_v0_12_scalar seckey_sum_scalar, addend, input_hash_scalar;
    rustsecp256k1_v0_12_ge prevouts_pubkey_sum_ge;
    rustsecp256k1_v0_12_gej prevouts_pubkey_sum_gej;
    unsigned char shared_secret[33];
    rustsecp256k1_v0_12_pubkey current_scan_pubkey;
    const rustsecp256k1_v0_12_pubkey *spend_pubkey_ptrs[1];
    rustsecp256k1_v0_12_xonly_pubkey *generated_output_ptrs[1];
    int ret, sum_is_zero;
    ...
    seckey_sum_scalar = rustsecp256k1_v0_12_scalar_zero;
    for (i = 0; i < n_plain_seckeys; i++) {
        ...
        if (!ret) {
            ...
            rustsecp256k1_v0_12_scalar_clear(&seckey_sum_scalar);
            return 0;
        }
        rustsecp256k1_v0_12_scalar_add(&seckey_sum_scalar, &seckey_sum_scalar, &addend);
    }
    ...
    sum_is_zero = rustsecp256k1_v0_12_scalar_is_zero(&seckey_sum_scalar);
    rustsecp256k1_v0_12_declassify(ctx, &sum_is_zero, sizeof(sum_is_zero));
    rustsecp256k1_v0_12_scalar_clear(&addend);
    if (sum_is_zero) {
        rustsecp256k1_v0_12_scalar_clear(&seckey_sum_scalar);
        return 0;
    }
    rustsecp256k1_v0_12_ecmult_gen(&ctx->ecmult_gen_ctx, &prevouts_pubkey_sum_gej, &seckey_sum_scalar);
    rustsecp256k1_v0_12_ge_set_gej(&prevouts_pubkey_sum_ge, &prevouts_pubkey_sum_gej);
     ...
     */
    rustsecp256k1_v0_12_declassify(ctx, &prevouts_pubkey_sum_ge, sizeof(prevouts_pubkey_sum_ge));
     ...
     * Returning an error here results in an untestable branch in the code, but we do this anyways to ensure strict compliance with BIP0352.
     */
    if (!rustsecp256k1_v0_12_silentpayments_calculate_input_hash_scalar(&input_hash_scalar, outpoint_smallest36, &prevouts_pubkey_sum_ge)) {
        rustsecp256k1_v0_12_scalar_clear(&seckey_sum_scalar);
        return 0;
    }
    rustsecp256k1_v0_12_scalar_mul(&seckey_sum_scalar, &seckey_sum_scalar, &input_hash_scalar);
    ...
    for (i = 0; i < n_recipients; i++) {
        if ((i == 0) || (rustsecp256k1_v0_12_ec_pubkey_cmp(ctx, &current_scan_pubkey, &recipients[i]->scan_pubkey) != 0)) {
            ...
            if (!rustsecp256k1_v0_12_pubkey_load(ctx, &pk, &recipients[i]->scan_pubkey)) {
                rustsecp256k1_v0_12_scalar_clear(&seckey_sum_scalar);
                /* Leaking this value would break indistinguishability of the transaction, so clear it. */
                rustsecp256k1_v0_12_memclear_explicit(&shared_secret, sizeof(shared_secret));
                return 0;
            }
            rustsecp256k1_v0_12_silentpayments_create_shared_secret(ctx, shared_secret, &pk, &seckey_sum_scalar);
            k = 0;
        }
        generated_output_ptrs[0] = generated_outputs[recipients[i]->index];
        spend_pubkey_ptrs[0] = &recipients[i]->spend_pubkey;
        if (!rustsecp256k1_v0_12_silentpayments_create_output_pubkeys(ctx, generated_output_ptrs, shared_secret, spend_pubkey_ptrs, 1, k)) {
            rustsecp256k1_v0_12_scalar_clear(&seckey_sum_scalar);
            rustsecp256k1_v0_12_memclear_explicit(&shared_secret, sizeof(shared_secret));
            return 0;
        }
...
int rustsecp256k1_v0_12_silentpayments_recipient_create_label(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_pubkey *label, unsigned char *label_tweak32, const unsigned char *scan_key32, const uint32_t m) {
    rustsecp256k1_v0_12_sha256 hash;
    ...
    rustsecp256k1_v0_12_sha256_write(&hash, scan_key32, 32);
    ...
    rustsecp256k1_v0_12_memclear_explicit(m_serialized, sizeof(m_serialized));
    rustsecp256k1_v0_12_sha256_clear(&hash);
    return rustsecp256k1_v0_12_ec_pubkey_create(ctx, label, label_tweak32);
}

int rustsecp256k1_v0_12_silentpayments_recipient_create_labeled_spend_pubkey(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_pubkey *labeled_spend_pubkey, const rustsecp256k1_v0_12_pubkey *unlabeled_spend_pubkey, const rustsecp256k1_v0_12_pubkey *label) {
    rustsecp256k1_v0_12_ge labeled_spend_pubkey_ge, label_addend;
     ...
     * return early
     */
    ret = rustsecp256k1_v0_12_pubkey_load(ctx, &labeled_spend_pubkey_ge, unlabeled_spend_pubkey);
    ret &= rustsecp256k1_v0_12_pubkey_load(ctx, &label_addend, label);
    if (!ret) {
        return 0;
    }
    rustsecp256k1_v0_12_gej_set_ge(&result_gej, &labeled_spend_pubkey_ge);
    ...
    rustsecp256k1_v0_12_ge_set_gej_var(&result_ge, &result_gej);
    rustsecp256k1_v0_12_pubkey_save(labeled_spend_pubkey, &result_ge);
...
int rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_create(
    const rustsecp256k1_v0_12_context *ctx,
    ...
    size_t i;
    rustsecp256k1_v0_12_ge prevouts_pubkey_sum_ge, addend;
    rustsecp256k1_v0_12_gej prevouts_pubkey_sum_gej;
     ...
     */
    rustsecp256k1_v0_12_gej_set_infinity(&prevouts_pubkey_sum_gej);
    for (i = 0; i < n_plain_pubkeys; i++) {
        if (!rustsecp256k1_v0_12_pubkey_load(ctx, &addend, plain_pubkeys[i])) {
            return 0;
        }
        rustsecp256k1_v0_12_gej_add_ge_var(&prevouts_pubkey_sum_gej, &prevouts_pubkey_sum_gej, &addend, NULL);
    }
    for (i = 0; i < n_xonly_pubkeys; i++) {
        if (!rustsecp256k1_v0_12_xonly_pubkey_load(ctx, &addend, xonly_pubkeys[i])) {
            return 0;
        }
        rustsecp256k1_v0_12_gej_add_ge_var(&prevouts_pubkey_sum_gej, &prevouts_pubkey_sum_gej, &addend, NULL);
    }
    if (rustsecp256k1_v0_12_gej_is_infinity(&prevouts_pubkey_sum_gej)) {
        return 0;
    }
    rustsecp256k1_v0_12_ge_set_gej_var(&prevouts_pubkey_sum_ge, &prevouts_pubkey_sum_gej);
    ...
    memcpy(&prevouts_summary->data[0], rustsecp256k1_v0_12_silentpayments_prevouts_summary_magic, 4);
    prevouts_summary->data[4] = 0;
    rustsecp256k1_v0_12_ge_to_bytes(&prevouts_summary->data[5], &prevouts_pubkey_sum_ge);
    rustsecp256k1_v0_12_scalar_get_b32(&prevouts_summary->data[5 + 64], &input_hash_scalar);
    return 1;
}

int rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_serialize(const rustsecp256k1_v0_12_context *ctx, unsigned char *output, size_t size, const rustsecp256k1_v0_12_silentpayments_prevouts_summary *prevouts_summary, unsigned int flags) {
    ...
    ARG_CHECK(rustsecp256k1_v0_12_memcmp_var(&prevouts_summary->data[0], rustsecp256k1_v0_12_silentpayments_prevouts_summary_magic, 4) == 0);
    if (flags == SECP256K1_EC_COMPRESSED) {
        compressed = 1;
    } else {
        compressed = 0;
     ...
     */
    rustsecp256k1_v0_12_ge_from_bytes(&ge, &prevouts_summary->data[5]);
    combined = (int)prevouts_summary->data[4];
    ret = 1;
    if (!combined) {
        rustsecp256k1_v0_12_scalar input_hash_scalar;
        rustsecp256k1_v0_12_scalar_set_b32(&input_hash_scalar, &prevouts_summary->data[5 + 64], NULL);
        ret &= rustsecp256k1_v0_12_eckey_pubkey_tweak_mul(&ge, &input_hash_scalar);
    }
    ret &= rustsecp256k1_v0_12_eckey_pubkey_serialize(&ge, output, &size, compressed);
    (void)ret;
    return 1;
}

int rustsecp256k1_v0_12_silentpayments_recipient_prevouts_summary_parse(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_silentpayments_prevouts_summary *prevouts_summary, const unsigned char *input, size_t inputlen) {
    rustsecp256k1_v0_12_ge pk;
    ...
    if (!rustsecp256k1_v0_12_eckey_pubkey_parse(&pk, input, inputlen)) {
        return 0;
    }
    ...
    rustsecp256k1_v0_12_ge_to_bytes(&prevouts_summary->data[5], &pk);
...
int rustsecp256k1_v0_12_silentpayments_recipient_scan_outputs(
    ...
    const rustsecp256k1_v0_12_silentpayments_prevouts_summary *prevouts_summary,
    const rustsecp256k1_v0_12_pubkey *spend_pubkey,
    const rustsecp256k1_v0_12_silentpayments_label_lookup label_lookup,
    const void *label_context
) {
    rustsecp256k1_v0_12_scalar output_tweak_scalar, scan_key_scalar;
    rustsecp256k1_v0_12_ge label_ge, spend_pubkey_ge, prevouts_pubkey_sum_ge;
    ...
    valid_scan_key = rustsecp256k1_v0_12_scalar_set_b32_seckey(&scan_key_scalar, scan_key32);
    rustsecp256k1_v0_12_declassify(ctx, &valid_scan_key, sizeof(valid_scan_key));
    if (!valid_scan_key) {
        rustsecp256k1_v0_12_scalar_clear(&scan_key_scalar);
        return 0;
    }
    rustsecp256k1_v0_12_ge_from_bytes(&prevouts_pubkey_sum_ge, &prevouts_summary->data[5]);
    combined = (int)prevouts_summary->data[4];
    if (!combined) {
        rustsecp256k1_v0_12_scalar input_hash_scalar;
        rustsecp256k1_v0_12_scalar_set_b32(&input_hash_scalar, &prevouts_summary->data[5 + 64], NULL);
        rustsecp256k1_v0_12_scalar_mul(&scan_key_scalar, &scan_key_scalar, &input_hash_scalar);
    }
    ret = rustsecp256k1_v0_12_pubkey_load(ctx, &spend_pubkey_ge, spend_pubkey);
    if (!ret) {
        rustsecp256k1_v0_12_scalar_clear(&scan_key_scalar);
        return 0;
    }
    rustsecp256k1_v0_12_silentpayments_create_shared_secret(ctx, shared_secret, &prevouts_pubkey_sum_ge, &scan_key_scalar);
    ...
    for (i = 0; i <= n_tx_outputs; i++) {
        rustsecp256k1_v0_12_ge output_ge = spend_pubkey_ge;
        /* Calculate the output_tweak and convert it to a scalar to ensure the value is less than the curve order.
         *
         * Note: _create_output_tweak can only fail if the output of the hash function is greater than or equal to the curve order, which is statistically improbable.
         * Returning an error here results in an untestable branch in the code, but we do this anyways to ensure strict compliance with BIP0352.
         */
        if (!rustsecp256k1_v0_12_silentpayments_create_output_tweak(&output_tweak_scalar, shared_secret, k)) {
            rustsecp256k1_v0_12_scalar_clear(&output_tweak_scalar);
            rustsecp256k1_v0_12_memclear_explicit(&shared_secret, sizeof(shared_secret));
            return 0;
        }
        ...
        found = 0;
        rustsecp256k1_v0_12_xonly_pubkey_save(&output_xonly, &output_ge);
        for (j = 0; j < n_tx_outputs; j++) {
            if (rustsecp256k1_v0_12_xonly_pubkey_cmp(ctx, &output_xonly, tx_outputs[j]) == 0) {
                label_tweak = NULL;
                found = 1;
                found_idx = j;
                break;
            ...
            if (label_lookup != NULL) {
                rustsecp256k1_v0_12_ge output_negated_ge, tx_output_ge;
                rustsecp256k1_v0_12_gej tx_output_gej, label_gej;
                unsigned char label33[33];
                size_t len;

                rustsecp256k1_v0_12_xonly_pubkey_load(ctx, &tx_output_ge, tx_outputs[j]);
                rustsecp256k1_v0_12_gej_set_ge(&tx_output_gej, &tx_output_ge);
                rustsecp256k1_v0_12_ge_neg(&output_negated_ge, &output_ge);
                rustsecp256k1_v0_12_gej_add_ge_var(&label_gej, &tx_output_gej, &output_negated_ge, NULL);
                rustsecp256k1_v0_12_ge_set_gej_var(&label_ge, &label_gej);
                ret = rustsecp256k1_v0_12_eckey_pubkey_serialize(&label_ge, label33, &len, 1);
                 ...
                 */
                rustsecp256k1_v0_12_gej_add_ge_var(&label_gej, &label_gej, &output_negated_ge, NULL);
                rustsecp256k1_v0_12_ge_set_gej_var(&label_ge, &label_gej);
                ret = rustsecp256k1_v0_12_eckey_pubkey_serialize(&label_ge, label33, &len, 1);
                /* Serialize must succeed because the point was just loaded.
                 *
        ...
        if (found) {
            ...
            rustsecp256k1_v0_12_scalar_get_b32(found_outputs[n_found]->tweak, &output_tweak_scalar);
            ...
            if (label_tweak != NULL) {
                 ...
                 */
                if (!rustsecp256k1_v0_12_ec_seckey_tweak_add(ctx, found_outputs[n_found]->tweak, label_tweak)) {
...
int rustsecp256k1_v0_12_silentpayments_recipient_create_output_pubkeys(const rustsecp256k1_v0_12_context *ctx, rustsecp256k1_v0_12_xonly_pubkey **outputs_xonly, const unsigned char *scan_key32, const rustsecp256k1_v0_12_silentpayments_prevouts_summary *prevouts_summary, const rustsecp256k1_v0_12_pubkey **spend_pubkeys, size_t n_spend_pubkeys)
{
    rustsecp256k1_v0_12_scalar scan_key_scalar;
    rustsecp256k1_v0_12_ge input_pubkey_ge;
    ...
    rustsecp256k1_v0_12_ge_from_bytes(&input_pubkey_ge, &prevouts_summary->data[5]);
    combined = (int)prevouts_summary->data[4];
    if (!combined) {
        rustsecp256k1_v0_12_scalar input_hash_scalar;
        rustsecp256k1_v0_12_scalar_set_b32(&input_hash_scalar, &prevouts_summary->data[5 + 64], NULL);
    ...
    rustsecp256k1_v0_12_silentpayments_create_shared_secret(ctx, shared_secret, &input_pubkey_ge, &scan_key_scalar);
    rustsecp256k1_v0_12_scalar_clear(&scan_key_scalar);
    return rustsecp256k1_v0_12_silentpayments_create_output_pubkeys(ctx, outputs_xonly, shared_secret, spend_pubkeys, n_spend_pubkeys, 0);
}
